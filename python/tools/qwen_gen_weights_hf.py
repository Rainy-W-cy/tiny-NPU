#!/usr/bin/env python3
"""Generate tiny-Qwen weights.bin from HuggingFace Qwen2-0.5B weights.
...
and packs the result into weights.bin.

Golden inference is intentionally handled by python/golden/qwen_infer_golden.py
so text prompt processing and numeric golden generation stay in the infer layer,
matching the GPT-style flow.
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate tiny-Qwen weights.bin from HuggingFace Qwen2-0.5B")
    ...
    print(
        ...
        f"--outdir {args.outdir} --prompt \"Hello\" --max-tokens 10"
    )
    print("  ./llama_demo_infer --datadir", args.outdir)
